Ext.require([ 'Ext.tree.Panel', 'Ext.tree.plugin.TreeViewDragDrop',
        'Ext.data.Model', 'Ext.data.TreeStore' ]);
...
Ext
        .onReady(function()
        {
            ...
            {
                return ' ' + nodeStatus.commandStr + ' ('
                        + nodeStatus.metaState + ') ' + nodeStatus.state;
            }
            ...
            QueueSpace._find_node = function(nodeID, parentNode)
            {
                ...
                while (childNode !== undefined && childNode !== null)
                {
                    ...
                    {
                        var grandchild = QueueSpace._find_node(nodeID,
                                childNode);
                        if (grandchild !== null)
                        {
                            return grandchild;
            ...
            QueueSpace.queue.on('added', function(nodes, parentID, siblingID)
            {
                ...
                if (parentNode == null)
                {
                    window.console.log("could not find", parentID,
                            QueueSpace.treeRoot);
                    return;
                }
            ...
            QueueSpace.queue.on('changed', function(nodeID, node_status)
            {
                ...
                {
                    treeNode.set('text', nodeID
                            + QueueSpace.generateText(node_status));
                }
